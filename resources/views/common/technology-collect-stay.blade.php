...
<div class="blade_container">
		...
		<div class="blade_slider" title="Select the amount to be treated.">
			...
			<label id = "collect-label-reduc" style="display:none;">Select a valid reduction rate between {{$tech->Nutri_Reduc_N_Low}}% and {{$tech->Nutri_Reduc_N_High}}%.</label>
			<input type="range" id="collect-rate" min="{{$tech->Nutri_Reduc_N_Low}}" max="{{$tech->Nutri_Reduc_N_High}}" v-model="collect_rate" value="{{$tech->Nutri_Reduc_N_Low}}" step="1" style="display:none;">
			<label id = "collect-label-rate" style="display:none;">@{{collect_rate}}%</label>
		</div>
