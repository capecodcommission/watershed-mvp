...
<!-- Set the popdown up with a header, a body with the technology, a table and reduction rate selection -->
	<div class="blade_container">
		...
		<button title="Delete Treatment" data-treatment="{{$treatment->TreatmentID}}" class="blade_button" v-show="fert_percent == {{$treatment->Treatment_Value}}" id="deletetreatment">Delete</button>
		<button title="Update Treatment" class="blade_button" v-show="fert_percent != {{$treatment->Treatment_Value}}" id="updatetreatment">Update</button>
	</div>
	...
	$(document).ready(function() {
		...
		$('#updatetreatment').on('click', function(e) {
			...
			if ("{{$tech->id == 25}}") {
				...
				let url = "{{url('/update/storm', $treatment->TreatmentID)}}" + '/' + percent;
				$.ajax({
					method: 'GET',
					url: url
				})
