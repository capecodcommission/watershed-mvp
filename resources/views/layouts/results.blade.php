			...
			<table>
				...
				<tbody>
					...
					<tr>
						...
						<td><?php  
							if($sub->n_load_scenario > $sub->n_load_target) {
								return round($sub->n_load_target - $sub->n_load_scenario);
							} else {
								return round($sub->n_load_scenario - $sub->n_load_target);
							} 
							?>
										
						</td><?php 
								if ($sub->n_load_scenario > $sub->n_load_target) {
									$n_rem_total += $sub->n_load_target - $sub->n_load_scenario;
								} else {
									$n_rem_total += $sub->n_load_scenario - $sub->n_load_target;
								}
								?>
					</tr>
					@endforeach
